...
import { callClaude, OPENROUTER_MODELS, DEFAULT_MODEL, getAgentModel, setAgentModel } from '@/lib/ai';
...
const MODEL_GROUPS = [
  { label: 'Gemini',   prefix: 'google/' },
  { label: 'Llama',    prefix: 'meta-llama/' },
...
function ModelDropdown({ agentId, disabled }: { agentId: string; disabled: boolean }) {
  ...
  const label = OPENROUTER_MODELS.find(m => m.id === selected)?.label ?? selected;

  // Group models
  const grouped = MODEL_GROUPS.map(g => ({
    ...g,
    models: g.prefix
      ? OPENROUTER_MODELS.filter(m => m.id.startsWith(g.prefix))
      : OPENROUTER_MODELS.filter(m => !MODEL_GROUPS.slice(0, -1).some(mg => m.id.startsWith(mg.prefix))),
  ...
  return (
    <>
      <button
        ...
        disabled={disabled}
        className="flex items-center gap-1.5 text-xs bg-gray-800 hover:bg-gray-700 disabled:opacity-40 text-gray-300 px-2.5 py-1.5 rounded-lg border border-gray-700 transition-colors min-w-[200px] justify-between"
      >
      ...
      {open && (
        ...
        >
          {grouped.map(group => (
            <div key={group.label}>
              <div className="px-3 pt-2.5 pb-1 text-[10px] font-semibold text-gray-500 uppercase tracking-wider">{group.label}</div>
              {group.models.map(m => (
                <button
                  key={m.id}
                  onClick={() => choose(m.id)}
