  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={disabled}
        className="flex items-center gap-1.5 text-xs bg-gray-800 hover:bg-gray-700 disabled:opacity-40 text-gray-300 px-2.5 py-1.5 rounded-lg border border-gray-700 transition-colors min-w-[160px] justify-between"
      >
        <span className="truncate">{label}</span>
        <ChevronDown size={11} className={`shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl overflow-hidden min-w-[220px]">
  ...
  async function runAgent(agent: Agent) {
    ...
    try {
      ...
      const friendly =
        ...
        msg.startsWith('MODEL_NOT_FOUND') ? `Model unavailable — pick a different one below` :
                                            `Agent failed: ${msg}`;
    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-1 gap-4">
        {agents.map(agent => {
          ...
          return (
            <div key={agent.id} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
